resource "aws_apigatewayv2_api" "startServer" {
  name          = "stop-${var.game_name}"
  protocol_type = "HTTP"
}

resource "aws_apigatewayv2_integration" "stopServer" {
  ...
  integration_uri     = aws_lambda_function.stopServer.invoke_arn
}
...
resource "aws_lambda_function" "stopServer" {
